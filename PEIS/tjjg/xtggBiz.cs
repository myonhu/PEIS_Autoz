...
namespace PEIS.main
{
    class xtggBiz :Base
    {
        ...
        {
            string strSql;
            strSql = "select czyid,czymc from xt_czy";
            return base.SqlDBAgent.GetDataTable(strSql);
        }
        public DataTable Get_xt_czy(string czyid)
        {
            string strSql;
            strSql = "select czyid,czymc,yhzid,mm,czybm,rysx,tybz,ksid from xt_czy where czyid='" + czyid + "'";
            return base.SqlDBAgent.GetDataTable(strSql);
        }
        public int Insert_xt_czy(string czyid, string czymc, string czybm, string yhzid, string mm, string rysx, string tybz, string ksid)
        {
            string strSql;
            strSql = "delete xt_czy where czyid='" + czyid + "';";
            strSql = strSql + "insert xt_czy(czyid,czymc,czybm,yhzid,mm,rysx,tybz,ksid) select '" + czyid + "','" + czymc + "','"
                + czybm + "','" + yhzid + "','" + mm + "','" + rysx + "','" + tybz + "','" + ksid + "'";
            return base.SqlDBAgent.sqlupdate(strSql);
        }
