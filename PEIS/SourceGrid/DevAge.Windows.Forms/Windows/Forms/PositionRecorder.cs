	...
	public class PositionRecorderIsolatedStorage : DevAge.IO.IsolatedStorage.IsolatedStorageSettingVersionBase
	{
		...
		public virtual void Save(Control p_Control)
		{
			if (p_Control is System.Windows.Forms.Form)
			{
				...
				// .NET Bug
				//se richiesto controllo se il form ?MDIChild. In questo caso vado a vedere quale ?il form attivo e como stato uso quello del form attivo.
				// questo perch?se ad esempio ho 2 form entrambi maximized, .net metto normal il form non attivo e quindi durante il close salvo lo stato come normal
				// anche se ad esempio cambio finestra con ctrl+tab il form si rimetta maximized. Secondo me ?un baco.
		...
		public virtual void Load(Control p_Control)
		{
			...
			{
				//se la form ?minimizzata non carico le impostazioni altrimenti risulterebbero sbagliate
			}
			else
			{
